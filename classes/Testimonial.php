...
class Testimonial
{
   ...
   */
  const DB_NAME = "testimonials";
  const CLASS_TYPE = "Testimonial";
  const IMAGE_PATH = TESTIMONIALS_IMAGE_PATH;
  ...
  * @return Testimonial|false The event object, or false if the record was not found or there was a problem
  */
  ...
  */
  ...
  public function insert() {
    ...
    // Insert the event
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "INSERT INTO " . self::DB_NAME . " ( person, title, summary, author, imageExtension ) VALUES ( :person, :title, :summary, :author, :imageExtension )";
    $st = $conn->prepare ( $sql );
  ...
  */
  ...
  public function update() {
    ...
    // Update the event
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "UPDATE " . self::DB_NAME . " SET person=:person, title=:title, summary=:summary, author=:author, imageExtension=:imageExtension WHERE id = :id";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":person", $this->person, PDO::PARAM_STR );
    $st->bindValue( ":title", $this->title, PDO::PARAM_STR );
    $st->bindValue( ":summary", $this->summary, PDO::PARAM_STR );
