...
class Sponsor
{
   ...
   */
  const DB_NAME = "sponsors";
  const CLASS_TYPE = "Sponsor";
  const IMAGE_PATH = SPONSORS_IMAGE_PATH;
  ...
  */
  ...
  public function __construct( $data=array() ) {
    ...
    if ( isset( $data['tier'] ) ) $this->tier = /*preg_replace ( "/[^\.\,\-\_\'\"\@\?\!\:\$ a-zA-Z0-9()]/", "",*/ $data['tier'] /*)*/;
  ...
  * @return Sponsor|false The event object, or false if the record was not found or there was a problem
  */
  ...
  */
  ...
  public function insert() {
    ...
    // Insert the event
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "INSERT INTO " . self::DB_NAME . " ( title, tier, url, author, imageExtension ) VALUES ( :title, :tier, :url, :author, :imageExtension )";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":title", $this->title, PDO::PARAM_STR );
    $st->bindValue( ":tier", $this->tier, PDO::PARAM_STR );
  ...
  */
  ...
  public function update() {
    ...
    // Update the event
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "UPDATE " . self::DB_NAME . " SET title=:title, tier=:tier, url=:url, author=:author, imageExtension=:imageExtension WHERE id = :id";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":title", $this->title, PDO::PARAM_STR );
    $st->bindValue( ":tier", $this->tier, PDO::PARAM_STR );
    $st->bindValue( ":url", $this->url, PDO::PARAM_STR );
