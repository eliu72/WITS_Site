...
class CommunityMember
{
   ...
   */
  const DB_NAME = "communityMembers";
  const CLASS_TYPE = "CommunityMember";
  const IMAGE_PATH = COMMUNITY_IMAGE_PATH;
  ...
  * @return CommunityMember|false The event object, or false if the record was not found or there was a problem
  */
  ...
  public static function getById( $id ) {
    ...
    $row = $st->fetch();
    $conn = null;
    if ( $row ) return new CommunityMember( $row );
  }
  ...
  */
  ...
  public static function getList( $numRows=1000000 ) {
    ...
    while ( $row = $st->fetch() ) {
      $communityMember = new CommunityMember( $row );
