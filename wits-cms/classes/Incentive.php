...
class Incentive
{
   ...
   */
  const DB_NAME = "incentives";
  const CLASS_TYPE = "Incentive";
  const IMAGE_PATH = INCENTIVES_IMAGE_PATH;
  ...
  */
  ...
  public static function getById( $id ) {
    ...
    $row = $st->fetch();
    $conn = null;
    if ( $row ) return new Incentive( $row );
  }
  ...
  */
  ...
  public function insert() {
    ...
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "INSERT INTO " . self::DB_NAME . " ( title, summary, author, imageExtension ) VALUES ( :title, :summary, :author, :imageExtension )";
    $st = $conn->prepare ( $sql );
  ...
  */
  ...
  public function update() {
    ...
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "UPDATE " . self::DB_NAME . " SET title=:title, summary=:summary, author=:author, imageExtension=:imageExtension WHERE id = :id";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":title", $this->title, PDO::PARAM_STR );
    $st->bindValue( ":summary", $this->summary, PDO::PARAM_STR );
    $st->bindValue( ":author", $this->author, PDO::PARAM_STR );
